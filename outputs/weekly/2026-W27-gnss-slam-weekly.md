# 每周 GNSS/融合/SLAM 热点汇总 | 2026-07-04

本周共聚合 5 篇日报候选论文，覆盖日期：2026-07-03。

## 热点方向

1. SLAM/里程计鲁棒性（4）
2. 多模态融合与传感器退化处理（3）
3. 可复现代码与开源实现（3）
4. GNSS 完整性与欺骗/干扰检测（1）
5. 数据集与 benchmark（1）

## 机器人领域热点雷达

1. 退化场景多模态融合与鲁棒里程计（4）
2. 3DGS/NeRF 神经场 SLAM（4）
3. 多机器人协同 SLAM 与分布式建图（4）
4. 韧性 PNT 与 GNSS 抗欺骗抗干扰（1）

## 高频主题

1. SLAM 与鲁棒里程计（4）
2. 多模态/多传感器融合（3）
3. GNSS 欺骗与干扰检测（1）

## 高频关键词

1. odometry（4）
2. loop（4）
3. gnss（3）
4. visual（3）
5. slam（3）
6. localization（3）
7. camera（2）
8. mapping（2）
9. gaussian（2）
10. inertial（2）
11. gps（1）
12. spoofing（1）
13. jamming（1）
14. interference（1）
15. integrity（1）
16. detection（1）

## 本周最值得追的论文

1. **AUSLUN: A Fixed-Hover UAV--USV System for GNSS-Denied Maritime Search and Navigation**
   - 质量分：14.0
   - 质量信号：质量分 14.0；venue NAVIGATION；代码开源；真实实验
   - 链接：http://arxiv.org/abs/2606.29875v1
2. **DL-VINS-Factory: A Modular Framework for Learned Visual Front-Ends in Visual-Inertial SLAM**
   - 质量分：8.5
   - 质量信号：质量分 8.5；代码开源；数据集/benchmark
   - 链接：http://arxiv.org/abs/2607.01757v1
3. **LXD-SLAM: LiDAR+X Dense SLAM with $\sum_{i=0}^{5}C_5^i$ Configurable Sensor Combinations**
   - 质量分：8.0
   - 质量信号：质量分 8.0；代码开源；真实实验
   - 链接：http://arxiv.org/abs/2606.27811v1
4. **GNSS Jamming Detection with Automatic Gain Control (AGC) and Carrier-to-Noise Ratio Density (CNO) Observables from a COTS receiver**
   - 质量分：0.0
   - 质量信号：质量分 0.0；venue 7th SmartRacon scientific seminar, Oct 2025, Stuttgart, Germany
   - 链接：http://arxiv.org/abs/2602.12688v1
5. **Self-supervised Geometry Reasoning for LiDAR Simultaneous Localization and Mapping**
   - 质量分：0.0
   - 质量信号：质量分 0.0；暂无外部质量元数据
   - 链接：http://arxiv.org/abs/2606.30166v1

## 有代码/复现线索

- **AUSLUN: A Fixed-Hover UAV--USV System for GNSS-Denied Maritime Search and Navigation**：质量分 14.0；venue NAVIGATION；代码开源；真实实验；http://arxiv.org/abs/2606.29875v1
- **DL-VINS-Factory: A Modular Framework for Learned Visual Front-Ends in Visual-Inertial SLAM**：质量分 8.5；代码开源；数据集/benchmark；http://arxiv.org/abs/2607.01757v1
- **LXD-SLAM: LiDAR+X Dense SLAM with $\sum_{i=0}^{5}C_5^i$ Configurable Sensor Combinations**：质量分 8.0；代码开源；真实实验；http://arxiv.org/abs/2606.27811v1

## 有 venue / 引用线索

- **AUSLUN: A Fixed-Hover UAV--USV System for GNSS-Denied Maritime Search and Navigation**：质量分 14.0；venue NAVIGATION；代码开源；真实实验；http://arxiv.org/abs/2606.29875v1
- **GNSS Jamming Detection with Automatic Gain Control (AGC) and Carrier-to-Noise Ratio Density (CNO) Observables from a COTS receiver**：质量分 0.0；venue 7th SmartRacon scientific seminar, Oct 2025, Stuttgart, Germany；http://arxiv.org/abs/2602.12688v1

## 编辑观察

本周最值得继续跟踪的是 SLAM/里程计鲁棒性、多模态融合与传感器退化处理、可复现代码与开源实现。放到更宽的机器人领域看，退化场景多模态融合与鲁棒里程计、3DGS/NeRF 神经场 SLAM、多机器人协同 SLAM 与分布式建图 的信号最强。关键词上，odometry、loop、gnss、visual、slam、localization 出现频率较高；后续选题可以优先挑有代码、真实实验或明确 venue/引用信号的论文做深度解读。

> 正式引用实验结论前，建议回到原文核对数据集、指标和实验设置。
