...
方法章通常解释作者怎样重新组织局部几何、匹配关系或地图表达。通俗地说，它是在告诉 SLAM 系统哪些点更可信、哪些约束更该相信，以及怎样避免错误匹配拖垮整条轨迹。文中这一段反复出现的线索是 LiDAR, IMU, SLAM。
...
结论部分要看可迁移性：新增几何模块是否能离开原数据集、原传感器配置，仍然稳定改善定位和建图。
